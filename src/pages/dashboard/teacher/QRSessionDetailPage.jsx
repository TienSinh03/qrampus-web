...
const QRSessionDetailPage = () => {
  ...
  const classSessionId = selectedSession?.class_session_id || selectedSchedule?.class_session_id || selectedSchedule?.id || null;
  const attendanceSessionId = selectedSession?.id || selectedSchedule?.attendanceSession?.id || null;

  const loadStats = useCallback(async () => {
    if (!attendanceSessionId) return false;

    const response = await getStats(attendanceSessionId);

    return Boolean(response);
  }, [attendanceSessionId, getStats]);
  ...
  const practiceGroupName = selectedSchedule?.practiceGroup?.group_name || (selectedSchedule?.schedule_type === "practice" ? "Chưa phân nhóm" : "Lý thuyết");
  ...
  const scheduleForResultPage = useMemo(() => {
    ...
    return {
      ...selectedSchedule,
      id: classSessionId || selectedSchedule?.id,
  ...
  const handleOpenResultPage = () => {
    ...
    navigate("/dashboard/results-qr", {
      state: { schedule: scheduleForResultPage },
    });
  };
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <h1 className="mt-2 text-2xl font-bold text-slate-900">{selectedSchedule?.courseSection?.name || "Chưa có học phần"}</h1>
            <div className="mt-2 flex flex-wrap items-center gap-3 text-sm text-slate-600">
              <span className="rounded-full bg-slate-100 px-3 py-1 font-medium">{selectedSchedule?.courseSection?.code || "---"}</span>
              <span className="rounded-full bg-blue-100 px-3 py-1 font-medium text-blue-700">{selectedSchedule?.schedule_type === "practice" ? "Thực hành" : "Lý thuyết"}</span>
              <span className="rounded-full bg-emerald-100 px-3 py-1 font-medium text-emerald-700">{practiceGroupName}</span>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm lg:col-span-4">
          ...
          <div className="mt-4 space-y-3 text-sm">
            <div className="flex items-center justify-between rounded-lg bg-slate-50 px-3 py-2">
              ...
              <span className="font-medium text-slate-800">{selectedSchedule?.courseSection?.semester || "Chưa cập nhật"}</span>
            </div>
            <div className="flex items-center justify-between rounded-lg bg-slate-50 px-3 py-2">
...
export default QRSessionDetailPage;
